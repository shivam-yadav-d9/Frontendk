// app/(delivery)/attendance.jsx
//
// ⚠️  This screen does NOT own tracking — (delivery)/_layout.jsx does.
//     attendance.jsx only LISTENS to events and reads local data.
//     Never call deliveryLocationService.startTracking()/stopTracking() here.
...
import deliveryAttendanceService from "../../services/deliveryAttendance.service";
...
export default function DeliveryAttendance() {
    ...
    const refreshAttendanceData = useCallback(async () => {
        try {
            const status = await deliveryAttendanceService.getCurrentStatus();
            const history = await deliveryAttendanceService.getAttendanceHistory();
            ...
            setActiveCheckIn(
                status === "CHECKED_IN" ? deliveryAttendanceService.openSessionCheckIn : null
            );
            if (history.success) setAttendanceHistory(history.data);

            const loc = await deliveryLocationService.getCurrentLocation();
            if (loc) {
                setDistance(loc.distance);
                setIsInsideOffice(loc.isInside);
            }
        } catch (e) {
            console.error("[DeliveryAttendance] refresh error:", e);
        }
    }, []);
    useEffect(() => {
        ...
        eventEmitter.on("DELIVERY_ATTENDANCE_UPDATED", handleAttendanceUpdate);
        eventEmitter.on("DELIVERY_LOCATION_UPDATED", handleLocationUpdate);

        refreshAttendanceData();

        return () => {
            eventEmitter.off("DELIVERY_ATTENDANCE_UPDATED", handleAttendanceUpdate);
            eventEmitter.off("DELIVERY_LOCATION_UPDATED", handleLocationUpdate);
        };
    }, [refreshAttendanceData]);

    const onRefresh = useCallback(async () => {
        setRefreshing(true);
        deliveryAttendanceService.clearStatusCache();
    ...
    const statusColor = isCheckedIn ? BLUE : RED;
                ...
                <View style={styles.statsRow}>
                    ...
                    <View style={styles.statCard}>
                        <MaterialIcons
                            ...
                            size={22}
                            color={isInsideOffice ? BLUE : ORANGE}
                        />
                        ...
                        <View style={styles.sessionCard}>
                            <View style={styles.sessionTopRow}>
                                ...
                                <View
                                    style={[
                                        styles.badge,
                                        { backgroundColor: isCheckedIn ? BLUE_LIGHT : RED_LIGHT },
                                    ]}
                                >
                                    <View
                            ...
                            <View style={styles.detailRow}>
                                ...
                                <Text
                                    style={[
                                        ...
                                        {
                                            color:
                                                todayDisplayStatus === "Present" ? BLUE : RED,
                                        },
                                    ]}
                                >
                                    {todayDisplayStatus}
