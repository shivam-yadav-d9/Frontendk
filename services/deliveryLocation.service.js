// services/deliveryLocation.service.js
//
// Foreground geofence tracker for the Delivery role.
// Same geofence math as the staff app (OFFICE_LOCATION / MAX_DISTANCE /
// calculateDistance from utils/location) but drives deliveryAttendance.service
...
import deliveryAttendanceService from "./deliveryAttendance.service";

const LAST_LOCATION_KEY = "deliveryLastLocation";
...
class DeliveryLocationService {
    ...
    async startTracking() {
        if (this.isTracking) {
            console.log("[DeliveryLocation] Already tracking");
            return true;
        }
        ...
        try {
            ...
            console.log("[DeliveryLocation] Tracking started");
            return true;
        } catch (e) {
            console.error("[DeliveryLocation] Failed to start:", e);
            this.isTracking = false;
            return false;
        }
    }
    ...
    stopTracking() {
        ...
        this.isTracking = false;
        console.log("[DeliveryLocation] Tracking stopped");
    }
    ...
    async handleLocationUpdate(location) {
        ...
        eventEmitter.emit("DELIVERY_LOCATION_UPDATED", {
        ...
        try {
            ...
            console.log(
                `[DeliveryLocation] dist=${Math.round(distance)}m inside=${isInside} checkedIn=${isCheckedIn}`
            );
            ...
            if (
                ...
                const result = await deliveryAttendanceService.checkIn(
                    location.coords.latitude,
                    location.coords.longitude
                );
                if (result.success) {
                    ...
                    eventEmitter.emit("DELIVERY_ATTENDANCE_UPDATED");
                }
            } else if (
                !isInside &&
                isCheckedIn &&
                ...
                const result = await deliveryAttendanceService.checkOut(
                    location.coords.latitude,
                    location.coords.longitude
                );
                if (result.success) {
            ...
            console.error("[DeliveryLocation] Error handling update:", e);
        } finally {
            this.isProcessing = false;
        }
    }
    ...
    handleAppStateChange(nextAppState) {
        if (
            ...
            console.log("[DeliveryLocation] App foregrounded — refreshing");
    ...
    async getCurrentLocation() {
        try {
            ...
            eventEmitter.emit("DELIVERY_LOCATION_UPDATED", result);
            await AsyncStorage.setItem(LAST_LOCATION_KEY, JSON.stringify(result));

            return result;
        } catch (e) {
            console.error("[DeliveryLocation] getCurrentLocation error:", e);
            return null;
        }
    }
}
